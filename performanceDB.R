...
if(FALSE)
{
  ...
  tmp=rawdata[1,]
  tmp=performanceDB.SQL.rawdatapreprocess(tmp);
  ...
  performanceDB.update(newdata=tmp,snames=snames,vnames=vnames)
  vnames
  snames
...
performanceDB.rightmodel=function(opttype)
{
  ...
  SpecDB=performanceDB.SQL.preprocess(SepcDB)
  #}
  
  
  
  trainingset=SpecDB[opttype]
{
